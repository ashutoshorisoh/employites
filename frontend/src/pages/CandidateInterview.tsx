...
import { 
  ClipboardList, CheckCircle2, Shield, Eye, ChevronRight, Sparkles, Check, Loader2, User, Mail, Video, Lock, FileText, UploadCloud, AlertCircle, LogOut, Download
} from 'lucide-react';
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-md glass-panel rounded-2xl p-8 bg-zinc-950 border border-zinc-900 shadow-2xl relative overflow-hidden">
          ...
          <div className="mb-6 text-center">
            <h3 className="text-lg font-extrabold text-white">Candidate Security Portal</h3>
            <p className="text-xs text-zinc-500 mt-1">Verify credentials to proceed securely.</p>
          </div>

          {authError && (
            <div className="p-3 bg-rose-950/30 border border-rose-500/20 text-rose-400 text-xs font-semibold rounded-xl mb-4 text-center">
              {authError}
            </div>
          )}
          
          {otpSentMsg && (
            <div className="p-3 bg-emerald-950/30 border border-emerald-500/20 text-emerald-400 text-xs font-semibold rounded-xl mb-4 text-center">
              {otpSentMsg}
            </div>
          )}
          ...
          {authStep === 'login' && (
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              <div>
                <label className="block text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-2">Email</label>
                <input
                  type="text"
                  disabled
                  value={authEmail}
                  className="w-full px-4 py-2.5 bg-zinc-900/40 border border-zinc-900 rounded-xl text-zinc-500 text-xs focus:outline-none"
                />
              </div>
              <div>
                <label className="block text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-2">Portal Password</label>
                <input
                  type="password"
                  required
                  value={authPassword}
              ...
              <button
                ...
                className="w-full text-center text-[10px] text-zinc-500 hover:text-white mt-2 block"
              >
          ...
          {authStep === 'register' && (
            <form onSubmit={handleRegisterSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-1.5">First Name</label>
                  <input
                    type="text"
                    required
                    value={authFirstName}
                ...
                <div>
                  <label className="block text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-1.5">Last Name</label>
                  <input
                    type="text"
                    required
                    value={authLastName}
              ...
              <div>
                <label className="block text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-1.5">Create Password</label>
                <input
                  type="password"
                  required
                  value={authPassword}
              ...
              <div>
                <label className="block text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-1.5">Verification Code</label>
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    disabled={isAuthSubmitting}
                    className="px-4 py-2.5 bg-zinc-900 border border-zinc-800 text-[10px] font-bold text-zinc-300 hover:text-white rounded-xl transition-all"
                  >
                    Send OTP
                  </button>
                </div>
              ...
              >
                {isAuthSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Register & Continue'}
              </button>
              <button
                type="button"
                onClick={() => setAuthStep('email')}
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-zinc-900 pb-5 mb-8 gap-4">
          <div>
            ...
            <h1 className="text-2xl font-extrabold text-white mt-2 tracking-tight">{previewJob?.title || 'Job Assessment'}</h1>
          ...
          <button
            ...
            className="px-4 py-2 bg-zinc-900 hover:bg-zinc-850 border border-zinc-800 rounded-xl text-xs font-bold text-zinc-400 hover:text-white transition-all"
          >
            Abandon Assessment
          </button>
        </div>
      ...
      <div className="min-h-[85vh] flex flex-col items-center justify-center p-6 md:p-12">
        <div className="w-full max-w-4xl glass-panel rounded-3xl p-10 md:p-14 relative overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.8)] border border-zinc-900/60 bg-zinc-950/95">
          ...
          <div className="flex justify-between items-start border-b border-zinc-900/65 pb-6 mb-8">
            <div>
              <span className="text-[10px] font-extrabold text-accentCyan bg-accentCyan/10 border border-accentCyan/20 px-3 py-1 rounded-md uppercase tracking-wider">
                Invitation Assessment Preview
              </span>
              <h2 className="text-3xl md:text-4xl font-black text-white tracking-tight mt-3.5 leading-tight">{previewJob.title}</h2>
            ...
            <span className={`text-[10.5px] font-extrabold px-3 py-1 rounded-md border uppercase tracking-wider ${
              isClosed 
          ...
          <div className="space-y-8">
            ...
            <div className="pt-8 border-t border-zinc-900/60 flex flex-wrap items-center justify-between gap-4">
              <button 
                onClick={() => navigate('/')}
                className="px-5 py-3 text-xs md:text-sm text-zinc-400 hover:text-white border border-zinc-850 hover:border-zinc-800 bg-zinc-900/20 hover:bg-zinc-900 font-extrabold transition-all rounded-xl cursor-pointer"
              >
                Go back
              </button>
              
              {isClosed ? (
                <div className="p-4 bg-rose-950/20 border border-rose-500/20 rounded-xl text-rose-400 text-xs md:text-sm font-semibold">
                  This job listing has been closed. No further submissions are accepted.
                </div>
              ) : (
                <button
                  onClick={() => {
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-zinc-900 pb-5 mb-8 gap-4">
          <div>
            ...
            <h1 className="text-2xl font-extrabold text-white mt-2 tracking-tight">Welcome, {user.name || user.email}!</h1>
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => {
                const tok = prompt("Enter job invite token (e.g. INV-XXXXXX):");
                if (tok) {
                  fetchJdByToken(tok);
                }
              }}
            ...
            >
              <Video className="w-4 h-4" /> Take Job Assessment
            </button>
            
            <button
              onClick={() => { logout(); resetInterviewForm(); }}
        ...
        {isLoadingApps ? (
                ...
                <div key={app.submission_id} className="glass-panel rounded-2xl p-6 relative overflow-hidden border border-zinc-900">
                  <div className="flex justify-between items-start flex-wrap gap-4 mb-4">
                    <div>
                      ...
                      <p className="text-[10px] text-zinc-500 font-mono mt-1">ID: {app.submission_id}</p>
                    </div>
                    ...
                    <span className={`text-[10px] font-extrabold px-3 py-1 rounded-xl border uppercase tracking-wider ${
                      status === 'Pending' 
                        ...
                        : isShortlisted
                        ? 'bg-emerald-950/40 text-emerald-400 border-emerald-500/20 shadow-[0_0_8px_rgba(16,185,129,0.1)]'
                        : 'bg-zinc-900 text-zinc-500 border-zinc-800'
                    }`}>
                      {status}
                    </span>
                  ...
                  <p className="text-xs text-zinc-400 mb-6 max-w-2xl">{app.job_description}</p>
                  
                  {/* Resume Upload Panel if Shortlisted / Requested */}
                  {app.resume_requested && (
                    <div className="pt-5 border-t border-zinc-900/60 bg-zinc-950/40 p-5 rounded-xl mt-4">
                      ...
                      {app.resume_url ? (
                        <div className="flex items-center justify-between bg-zinc-900/50 border border-emerald-500/20 p-3 rounded-lg">
                          <div className="flex items-center gap-2 text-xs text-emerald-400 font-semibold">
                            <Check className="w-4 h-4" /> Resume Uploaded & Shared
                          </div>
                          <span className="text-[10px] text-zinc-500 font-mono">Location: {app.resume_url.split('_').slice(1).join('_') || 'resume.pdf'}</span>
                        </div>
                      ) : (
                        <div>
                          ...
                          <label className="relative border border-dashed border-zinc-800 hover:border-accentCyan/50 bg-zinc-950/60 rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer transition-all">
                            ...
                            {isUploadingResume && uploadingSubId === app.submission_id ? (
                              ...
                              <>
                                ...
                                <span className="text-[9px] text-zinc-500 mt-1">Files limited to PDF, maximum 5MB</span>
                              </>
                            )}
                          </label>
                        </div>
          ...
          <div className="glass-panel rounded-2xl p-10 text-center">
            ...
            <p className="text-xs text-zinc-500">No screen submissions registered.</p>
            <p className="text-[10px] text-zinc-600 mt-1">If you have a job invite code, click the top right button to take an assessment.</p>
          </div>
        )}
        {renderAuthModal()}
      </div>
        ...
        <div className="glass-panel rounded-2xl p-8 relative overflow-hidden">
          ...
          <div className="mb-6 text-center">
            <h2 className="text-xl font-extrabold text-white flex items-center justify-center gap-1.5">
              Candidate Workspace <Sparkles className="w-4 h-4 text-accentCyan" />
            </h2>
            <p className="text-xs text-gray-400 mt-1">Access interview assessments or view your shortlist reports.</p>
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 bg-zinc-950/60 border border-zinc-800 focus:border-accentCyan/50 rounded-xl text-gray-100 placeholder-zinc-600 focus:outline-none text-xs font-mono font-bold"
                />
                <button
                  onClick={() => fetchJdByToken(tokenInput)}
                  disabled={isJdLoading}
                  className="px-4 py-2 bg-zinc-900 hover:bg-zinc-850 border border-zinc-800 text-xs font-bold text-zinc-300 hover:text-white rounded-xl transition-all"
                >
                  Verify
                </button>
              </div>
            ...
            <div className="pt-4 border-t border-zinc-900/60 text-center">
              <span className="text-xs text-zinc-500 block mb-3">Or check applied status:</span>
              <button
                onClick={() => {
                  setAuthStep('email');
                  setAuthEmail('');
